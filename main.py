...
top_margin = int(window_w -(cup_h * block)-5)
...
def runTetris():
    ...
    going_right = False
    poinst = 0
    lvl, fall_speed = calcSpeed(poinst)
    ...
    while True:
        ...
        for event in pg.event.get():
            if event.type == KEYUP:
                if event.key == K_SPACE:
                    ...
                    showText('Пауза')
                    last_fall = time.time()
                    lvl_move_down = time.time()
                    lvl_move_side = time.time()
        ...
        if (going_left or going_right) and time.time() - last_side_move > side_freq:
            if going_left and checkPos(cup, fallingFig, adjY=-1):
                going_left['x'] -= 1
            if going_right and checkPos(cup, fallingFig, adjY=-1):
                going_left['x'] += 1
            last_move_down = time.time()
        ...
        if time.time() - last_fall >fall_speed:
            if not checkPos(cup, fallingFig, adjY=1):
                ...
                poinst += clearCompleted(cup)
                level, fall_speed = calcSpeed(poinst)
                fallingFig = None
            else:
                fallingFig['y'] +=1
                last_fall = time.time()
...
def showText(text):
    ...
    pressKeySurf, pressKeyRect = txtObjects('Нажмите любую клавишу для продолжения',
...
def convertCoords(block_x, block_y):
    return (side_margin + (block_x * block)), (top_margin + (block_y *
    block))
...
def drawBlock(block_x, block_y, color, pixelx=None, pixely=None):
    ...
    if pixelx == None and pixely == None:
        ...
        pg.draw.rect(display_surf, colors[color], (pixelx + 1, pixely + 1, block - 1, block -
        1), 0, 3)
        pg.draw.rect(display_surf, lightcolors[color], (pixelx + 1, pixely + 1, block - 4,
        block - 4), 0, 3)
        pg.draw.circle(display_surf, colors[color], (pixelx + block / 2, pixely))


def gamecup(cup):
    # граница игрового поля-стакана
    pg.draw.rect(display_surf, brd_color, (side_margin - 4, top_margin - 4,
    (cup_w * block) + 8, (cup_h * block) + 8), 5)

    # фон игрового поля
    pg.draw.rect(display_surf, bg_color, (side_margin, top_margin, block *
    cup_w, block * cup_h))
    for x in range(cup_w):
...
def drawnextFig(fig): # превью следующей фигуры
    nextSurf = basic_font.render('Следующая:', True,
    txt_color)
